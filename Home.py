...
type = st.radio(
    "",
    ["Émissions annuelles moyennes de GES (Gaz à Effet de Serre) des trajets domicile-travail selon le département de résidence en 2019", 
     "Distance domicile-travail moyenne (Km) selon le département de résidence en 2019", 
     "Émissions annuelles moyennes de GES (Gaz à Effet de Serre) des voitures par adulte selon le département en 2019"])

def importer_data(path):
      return 

if type == "Émissions annuelles moyennes de GES (Gaz à Effet de Serre) des trajets domicile-travail selon le département de résidence en 2019":
    df = pd.read_excel("co2_deplacements.ods", engine="odf", sheet_name="Figure_1", header=3)
    color = "Viridis"
elif type == "Distance domicile-travail moyenne (Km) selon le département de résidence en 2019":
...
fig = go.Figure(go.Choroplethmapbox(geojson=countries,locations=df['Numéro de département'], z=df[df.columns[2]],
                                    colorscale=color , zmin=min(df[df.columns[2]]), zmax=max(df[df.columns[2]]),
                                    marker_opacity=0.6, marker_line_width=0.5 ))
